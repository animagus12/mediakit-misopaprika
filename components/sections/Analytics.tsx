...
export default function Analytics() {
  const ranges = analyticsData.ranges
  const [range, setRange] = useState(ranges[0])

  const base = analyticsData.platforms
  const mul = range / analyticsData.ranges[0]
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">{analyticsData.title}</h3>
          <p className="text-sm text-muted-foreground">{analyticsData.description}</p>
        </div>
